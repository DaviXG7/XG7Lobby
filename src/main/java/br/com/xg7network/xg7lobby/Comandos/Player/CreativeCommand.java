...
public class CreativeCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (sender instanceof Player) {
            ...
            if (p.hasPermission("xg7lobby.gamemode.creative") || p.hasPermission("xg7lobby.admin")) {
                if (args.length == 0) {
                    ...
                    if (p.hasPermission("xg7lobby.gamemode.others")) {
                        ...
                        if (target != null) {
                            if (target.isOnline()) {
                                ...
                                if (mensagem.getMessage().getBoolean("mensagens.ativar_mensagem_modo_de_jogo")) {
                                    action.mandarAction(target, mensagem.getMessage().getString("mensagens.modo_criativo"));
                                }
                            } else {
