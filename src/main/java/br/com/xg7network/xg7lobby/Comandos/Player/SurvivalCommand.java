...
public class SurvivalCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (sender instanceof Player) {
            ...
            if (p.hasPermission("xg7lobby.gamemode.survival") || p.hasPermission("xg7lobby.admin")) {
                if (args.length == 0) {
                    if (!p.getGameMode().equals(GameMode.SURVIVAL)) {
                        p.setGameMode(GameMode.SURVIVAL);
                        if (mensagem.getMessage().getBoolean("mensagens.ativar_mensagem_modo_de_jogo")) {
                            action.mandarAction(p, mensagem.getMessage().getString("mensagens.modo_sobrevivência"));
                        }
                    } else {
                        if (mensagem.getMessage().getBoolean("mensagens.ativar_mensagem_modo_de_jogo")) {
                    ...
                    if (p.hasPermission("xg7lobby.gamemode.others")) {
                        ...
                        if (target != null) {
                            if (target.isOnline()) {
                                ...
                                if (mensagem.getMessage().getBoolean("mensagens.ativar_mensagem_modo_de_jogo")) {
                                    action.mandarAction(target, mensagem.getMessage().getString("mensagens.sobrevivência"));
                                }
                            } else {
