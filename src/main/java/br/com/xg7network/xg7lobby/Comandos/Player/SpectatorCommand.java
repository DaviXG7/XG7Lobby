            ...
            if (p.hasPermission("xg7lobby.command.creative") ||
                ...
                if (mensagem.getMessage().getBoolean("mensagens.ativar_mensagem_modo_de_jogo")) {
                    if (mensagem.getMessage().getBoolean("AvisoEmActionBars")) {
                        p.spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText(mensagem.getMessage().getString("mensagens.modo_espectador").replace("&", "§")));
                    } else {
                        p.sendMessage(mensagem.getMessage().getString("mensagens.modo_espectador").replace("&", "§"));
                    }
                }
            }else {
