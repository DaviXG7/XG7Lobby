...
public class BanCommand implements ICommand {
    ...
    public void onCommand(CommandSender sender, CommandArgs args) {
        ...
        Text.fromLang(sender, XG7Lobby.getInstance(), "commands.ban.on-ban-sender").thenAccept(text -> text.replace("target", target.getName()).replace("reason", reason).replace("time", String.format("%dd, %02dh %02dm %02ds", days, finalHours, finalMinutes, finalSeconds)).send(sender));
