...
public class SelectorGuidePage extends HelpPage {
    public SelectorGuidePage() {
        super("selector-guide");
        addMessages(
                HelpComponent.of(
                        XG7Lobby.getInstance(),
                        "&m-&9&m-&6&m------------------&e*&6&m------------------&9&m-&f&m-"
                ).build(),
                new SelectorGuidePageComponent(),
                HelpComponent.empty(),
                HelpComponent.of(
                        XG7Lobby.getInstance(),
                        "lang:[help.chat.back]"
    ...
    private class SelectorGuidePageComponent extends HelpComponent {
        public SelectorGuidePageComponent() {
        ...
        public Component buildFor(Player player) {
            ...
            String about = lang.getList("help.selector-guide", String.class).orElse(new ArrayList<>()).stream().collect(Collectors.joining("\n"));

            return Component.text(Text.detectLangs(player, XG7Plugins.getInstance(),about).join().getText()).build();
